...
| test_widget_tree_v3.py   | ToS_scanner    | Display yaml pseudo-widget hierarchy tree <br>                          ``` python test_widget_tree_v3.py layout_4real9.yaml ```            |
| widget_check.py          | toolkit/ToS_gui_survey | Draws hierarchy bboxes for a specified widget  <br>  ```python widget_check.py <some_pseudo-widget>```  <br> ```python widget_check.py ``` &emsp; [ -- pops up selection dialog -- ]     |
| test_tos_debug_panel.py  | ToS_scanner    | Presents GUI window <br>facilitating a variety of pseudo-widget actions |
