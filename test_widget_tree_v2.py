...
def normalize_top_level_roots(layout_obj: Any, source_name: str) -> list[tuple[str, Any]]:
    ...
    if isinstance(layout_obj, dict):
        ...
        # Fallback in case every node somehow appears as a child.
        if not roots:
            for key, node in all_items:
                root_name = extract_name(node, fallback=str(key))
                roots.append((root_name, node))
...
def format_node_line(node_name: str, node: Any) -> str:
    ...
    return f"{node_name}  (w={width}, h={height})"
...
def build_title_map() -> dict[str, str]:
    """
    Build a best-effort pseudo-window-name -> real window title map
    from WindowConfig.
    """
    cfg = WindowConfig()
    ...
    for attr_name in dir(cfg):
        ...
        title_map.setdefault(pseudo_name, value)
...
def main() -> int:
    ...
    logger = setup_logger("widget_tree_test_v2", "widget-tree-test-v2", log_dir)
